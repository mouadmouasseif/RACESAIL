...
export async function downloadPdf(competition: Competition) {
  ...
  const [raceSailLogo, federationLogo, clubLogo, competitionLogo] = await Promise.all([
    imageToBase64("/logo-racesail.png"),
    imageToBase64("/federation.png"),
    imageToBase64(competition.clubLogo ?? ""),
    imageToBase64(competition.competitionLogo ?? ""),
  ]);
  ...
  await Promise.all(rows.map(async (athlete) => {
    ...
    if (!flagImages.has(code)) flagImages.set(code, await imageToBase64(getFlagImagePath(athlete.nationality)));
  }));
  const clubImages = new Map<string, string | null>();
  await Promise.all(rows.map(async (athlete) => {
    if (!athlete.clubLogo || clubImages.has(athlete.clubName)) return;
    clubImages.set(athlete.clubName, await imageToBase64(athlete.clubLogo));
  }));

  addLogo(doc, raceSailLogo ?? APP_LOGO_BASE64, 14, 10);
  addLogo(doc, competitionLogo ?? clubLogo ?? undefined, 36, 10);
  addLogo(doc, federationLogo ?? FEDERATION_LOGO_BASE64, pageWidth - 32, 10);
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data: { section: string; column: { index: number }; row: { index: number }; cell: { x: number; y: number; height: number } }) => {
      ...
      if (data.column.index === 1) {
        ...
        if (image) addTinyImage(doc, image, data.cell.x + 1, data.cell.y + 1.2, 4.8, 3.2);
      }
      if (data.column.index === 2) {
        const image = clubImages.get(athlete.clubName);
        if (image) addTinyImage(doc, image, data.cell.x + 1, data.cell.y + 1, 4, 4);
      }
    },
  });
...
function addTinyImage(doc: jsPDF, image: string, x: number, y: number, width: number, height: number) {
  try {
    if (!image.startsWith("data:image")) return;
    const format = image.includes("image/png") ? "PNG" : image.includes("image/jpeg") || image.includes("image/jpg") ? "JPEG" : "PNG";
    doc.addImage(image, format, x, y, width, height);
  } catch {
    // Keep country/club text visible if a tiny image cannot be rendered.
  }
}
